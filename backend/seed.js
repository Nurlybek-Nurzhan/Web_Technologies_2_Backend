...
const commentsData = [
  { text: 'Amazing program! The trainers are incredibly knowledgeable.', rating: 5 },
  { text: 'Great value for money. Highly recommend for beginners.', rating: 4 },
  { text: 'Challenging but rewarding. I have seen real results!', rating: 5 },
  { text: 'Good variety of exercises. Keeps things interesting.', rating: 4 },
  { text: 'The flexibility sessions have really helped my recovery.', rating: 5 }
];
...
const seedDatabase = async () => {
  try {
    ...
    // Insert programs with createdBy reference to admin
    const programsWithRef = programsData.map(p => ({ ...p, createdBy: adminUser._id }));
    const programs = await Program.insertMany(programsWithRef);
    console.log(`Inserted ${programs.length} programs (linked to admin user)`);

    // Insert workouts with createdBy reference to admin
    const workoutsWithRef = workoutsData.map(w => ({ ...w, createdBy: adminUser._id }));
    const workouts = await Workout.insertMany(workoutsWithRef);
    console.log(`Inserted ${workouts.length} workouts (linked to admin user)`);

    // Insert comments linked to users and programs
    const commentDocs = commentsData.map((c, i) => ({
      text: c.text,
      rating: c.rating,
      user: i % 2 === 0 ? regularUser._id : adminUser._id,
      program: programs[i % programs.length]._id
    }));
    const comments = await Comment.insertMany(commentDocs);
    console.log(`Inserted ${comments.length} comments (linked to users and programs)`);

    console.log('\nDatabase seeded successfully!');
    console.log('Relational integrity:');
    console.log('  - Programs -> createdBy -> User');
    console.log('  - Workouts -> createdBy -> User');
    console.log('  - Comments -> user -> User, program -> Program');
    process.exit(0);
  } catch (error) {
