...
export const getPrograms = async (req, res) => {
  try {
    const programs = await Program.find().populate('createdBy', 'email role').sort({ createdAt: -1 });
    res.json(programs);
  } catch (error) {
...
export const getProgramById = async (req, res) => {
  try {
    const program = await Program.findById(req.params.id).populate('createdBy', 'email role');
